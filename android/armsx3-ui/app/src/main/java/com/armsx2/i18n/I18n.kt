...
val EN: Map<String, String> = mapOf(
    ...
    "cloud.section.title" to "Cloud Sync",
    "cloud.section.description" to "Mirror your save data and play games from the cloud. Saves sync over WebDAV (or any server that keeps a file tree); games stream from the same host's games/ folder and launch straight off the device.",
    "cloud.providers.label" to "No WebDAV server? Pick a free host",
    "cloud.providers.hint" to "Tapping a provider fills the address below — you only add your account's username and password. pCloud (10 GB), Koofr (10 GB), Mail.ru Cloud and Yandex Disk have free WebDAV tiers; Nextcloud is for your own instance. Google Drive, Dropbox and GoFile are API-only (no WebDAV) and would need developer app keys.",
    "cloud.provider.pcloud" to "pCloud",
    "cloud.provider.koofr" to "Koofr",
    "cloud.provider.mailru" to "Mail.ru",
    "cloud.provider.yandex" to "Yandex",
    "cloud.provider.nextcloud" to "Nextcloud",
    "cloud.provider.pcloud.hint" to "✓ pCloud address filled. Use the email and password of your pCloud account (sign up at pcloud.com; optionally generate an app password under Security).",
    "cloud.provider.koofr.hint" to "✓ Koofr address filled. Use your Koofr account email and password (sign up at app.koofr.net).",
    "cloud.provider.mailru.hint" to "✓ Mail.ru Cloud address filled. Use your full @mail.ru (or @bk.ru/@list.ru) mailbox login and password, or better an app password from Mail.ru settings.",
    "cloud.provider.yandex.hint" to "✓ Yandex Disk address filled. Yandex rejects the main password for WebDAV — create an app password in id.yandex.ru/profile/security and enter your Yandex login + that app password.",
    "cloud.provider.nextcloud.hint" to "Nextcloud needs your own URL: replace your-nextcloud.example and username with your server and login, then add the username and password below.",
    "cloud.url.label" to "Server address",
    "cloud.url.description" to "A WebDAV folder you own (Nextcloud, nginx+davfs, a sync directory\u2026). Saves land under saves/ and games stream from games/.",
    "cloud.url.fieldLabel" to "WebDAV/HTTP URL",
    "cloud.user.label" to "Username",
    "cloud.user.description" to "Leave blank for an anonymous server.",
    "cloud.user.fieldLabel" to "Username",
    "cloud.pass.label" to "Password",
    "cloud.pass.description" to "Stored in the app's private storage; sent as Basic auth.",
    "cloud.pass.fieldLabel" to "Password",
    "cloud.save" to "Save settings",
    "cloud.status.saved" to "Cloud sync configured.",
    "cloud.status.disabled" to "Cloud sync disabled.",
    "cloud.autopush" to "Sync saves when a game exits",
    "cloud.autopush.description" to "Uploads every save folder after the emulator shuts down, and again on app close, while a server is configured.",
    "cloud.push" to "Sync now",
    "cloud.pull" to "Pull saves",
    ...
    "cloud.games.file.description" to "Path on the server under games/, e.g. BLUS30475/BLUS30475.iso",
    "cloud.games.file.fieldLabel" to "Cloud game path",
    "cloud.games.launch" to "Download & play",
    "cloud.games.stream" to "Stream (no download)",
    "cloud.games.stream.description" to "Opens the game directly from your server over HTTP without downloading. Requires the server to support Range requests (nginx, Apache, etc.). Works best with a VPS on the same network as your device.",
    "cloud.games.stream.working" to "Connecting to server\u2026",
    "cloud.games.stream.done" to "Streaming from server.",
    "cloud.games.stream.failed" to "Stream failed. Is the server reachable?",
    "cloud.games.install" to "Install PKG (no download)",
    "cloud.games.install.description" to "Streams a .pkg straight from your server and installs it into dev_hdd0/game in one pass, so the package is never stored on your device twice. Requires the server to support Range requests (nginx, Apache, etc.).",
    "cloud.games.install.working" to "Installing from server\u2026",
    "cloud.games.install.done" to "PKG installed.",
    "cloud.games.install.failed" to "Install failed. Check the address, path, and that the server supports Range requests.",
    "cloud.download.working" to "Downloading\u2026",
    "cloud.download.done" to "Downloaded \u2014 launching.",
    "cloud.download.failed" to "Download failed. Check the address and path, or whether the file exists.",
    "github.section.title" to "GitHub save sync",
    "github.section.description" to "Push and pull saves to a private GitHub repository under your own account. The repo is created automatically if it does not exist yet.",
    "github.token.label" to "Personal access token",
    "github.token.description" to "A GitHub PAT with repo scope. Stored locally and sent only to api.github.com; never shared with any third-party server.",
    "github.token.field" to "Paste GitHub PAT",
    "github.repo.label" to "Repository name",
    "github.repo.description" to "A private repo by this name is created under the token's account on first sync, if it does not already exist.",
    "github.repo.field" to "armsx3-saves",
    "github.save" to "Save",
    "github.verify" to "Check token",
    "github.verify.ok" to "Server will follow account: %s",
    "github.verify.fail" to "GitHub could not authenticate this token.",
    "github.account" to "Account: %s",
    "github.autopush" to "Sync saves when a game exits",
    "github.push" to "Push saves",
    "github.pull" to "Pull saves",
    "github.push.working" to "Pushing\u2026",
    "github.pull.working" to "Pulling\u2026",
    "github.push.ok" to "Uploaded %d save(s) to GitHub.",
    "github.pull.ok" to "Downloaded %d save(s) from GitHub.",
    "github.push.fail" to "GitHub push failed.",
    "github.pull.fail" to "GitHub pull failed.",
    "github.disabled" to "GitHub sync disabled.",
    "github.ready" to "GitHub sync configured.",
    "preset.title" to "Performance preset",
    "preset.description" to "Apply a ready-made RPCS3 core profile. Everything below takes effect on the running core immediately and is saved when the emulator exits; it can be undone with Balanced or per-game settings.",
    "preset.balanced" to "Balanced",
    "preset.balanced.desc" to "Default-safe accuracy. Same as the shipped settings.",
    "preset.performance" to "Performance",
    "preset.performance.desc" to "Swaps accuracy for speed on every setting known to cost on ARM64. Recoverable with Balanced.",
    "preset.maximum" to "Maximum",
    "preset.maximum.desc" to "Fastest ARM64 backend settings. Expect visual glitches in some titles; only for validated games.",
    "preset.apply" to "Apply",
    "preset.apply.confirm" to "Apply this preset now? It rewrites these core settings and they are saved on exit.",
    "preset.applied" to "Preset applied.",
    "preset.failed" to "Preset did not fully apply — some settings were rejected.",
    ...
    "bios.firmware.dirInvalid" to "That folder was not accepted: it must be writable and contain dev_flash/vsh/module/vsh.self (installed firmware).",
